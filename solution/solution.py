...
def format_comparison_table(results: list[dict]) -> str:
    ...
    for r in results:
        rows.append([
            truncate(str(r.get("prompt", ""))),
            truncate(str(r.get("gpt4o_response", ""))),
            truncate(str(r.get("mini_response", ""))),
            f"{float(r.get('gpt4o_latency', 0.0)):.2f}s",
            f"{float(r.get('mini_latency', 0.0)):.2f}s",
        ])
